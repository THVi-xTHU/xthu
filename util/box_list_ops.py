...
def intersection(boxlist1, boxlist2):
  ...
  """
  return box_ops.intersection(boxlist1.get(), boxlist2.get())
...
def iou(boxlist1, boxlist2):
  ...
  """
  return box_ops.iou(boxlist1.get(), boxlist2.get())
...
def ioa(boxlist1, boxlist2):
  ...
  """
  return box_ops.ioa(boxlist1.get(), boxlist2.get())
  ...
  for i in range(num_boxes):
    if num_output < max_output_size:
      if is_index_valid[i]:
        ...
        intersect_over_union = box_ops.iou(
...
def multi_class_non_max_suppression(boxlist, score_thresh, iou_thresh,
  ...
  for class_idx in range(num_classes):
    boxlist_and_class_scores = box_list.BoxList(boxlist.get())
...
def scale(boxlist, y_scale, x_scale):
  ...
  x_min = x_scale * x_min
  x_max = x_scale * x_max
  scaled_boxlist = box_list.BoxList(np.hstack([y_min, x_min, y_max, x_max]))
