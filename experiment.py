...
from save_result import to_result
from _eval.label_process import to_json
...
def keep_box_list(box_list, field, val_list):
  ...
  #import pdb
  #pdb.set_trace()
  keep_ind = reduce(np.logical_or, [values == val for val in val_list])
...
#     test_zebra_contours(args.video_path, args.save_path)
    ...
    videos = [args.video_path + '/' + f.split('.')[0] \
